...
import type { PaginatedSprints } from './sprint.types.js';
...
export const createSprint = async (data: any, user: IUser): Promise<ISprint> => {
  const targetProjId = data.projectId || data.project;
  await checkSprintProjectAccess(targetProjId, user);
...
export const getSprints = async (
  ...
  const targetProjId = query.projectId || query.project;
  if (targetProjId) dbQuery.project = targetProjId;
...
export const updateSprint = async (id: string, data: any, user: IUser): Promise<ISprint> => {
  ...
  const allowed = ['name', 'goal', 'startDate', 'endDate', 'members', 'retrospective', 'archived', 'tasks'];
...
export const startSprint = async (id: string, user: IUser): Promise<ISprint> => {
  ...
  // Sync tasks status update if needed
  await Task.updateMany({ _id: { $in: sprint.tasks } }, { sprintId: sprint._id });
...
export const endSprint = async (id: string, retrospective: string | undefined, user: IUser): Promise<ISprint> => {
  ...
  // velocity = ratio of story points delivered vs planned
...
export const manageSprintTasks = async (
  ...
  if (action === 'add') {
    ...
    // Set sprintId on assigned tasks
    await Task.updateMany({ _id: { $in: taskIds } }, { sprintId: sprint._id });
  } else {
    sprint.tasks = sprint.tasks.filter((t) => !taskIds.includes(t.toString())) as any;
    // Remove sprintId on unassigned tasks
    await Task.updateMany({ _id: { $in: taskIds } }, { $unset: { sprintId: '' } });
  }
